...
- (id)initWithView:(PLViewBase *)aView scene:(PLScene *)aScene;
{
	...
	if(self)
	{
        ...
        if (context) 
        {
            context = [[EAGLContext alloc] initWithAPI:kEAGLRenderingAPIOpenGLES1 sharegroup:context.sharegroup];
        }
        else
        {
...
{
	UIDeviceOrientation orientation = view.isDeviceOrientationEnabled ? deviceOrientation : [view currentDeviceOrientation];
