...
- (void)viewDidLoad
{
    ...
    [self.view insertSubview:menuView aboveSubview:attributeView];
...
//
-(void)updataProductAttributeWithIndex:(uint)index
{
    NSMutableDictionary *temp = [products objectAtIndex:index];
    
    if (![temp objectForKey:@"setAllAttribute"])
    {
        [temp setObject:[NSNumber numberWithBool:YES] forKey:@"setAllAttribute"];
        
        //取配色
        id color = [Access getProductColorWithId:[temp objectForKey:@"id"]];
        
        [temp setValue:color forKey:@"color"];
        
        //取皮料
        id leather = [Access getProductLeatherWithId:[temp objectForKey:@"id"]];
        
        [temp setValue:leather forKey:@"leather"];
        
        //取规格
        id specification = [Access getProductSpecificationWithId:[temp objectForKey:@"id"]];
        
        [temp setValue:specification forKey:@"specification"];
        
        //取相册
        NSMutableArray *album = [Access getProductAlbumWithId:[temp objectForKey:@"id"]];
        
        //取实景相册
        NSMutableArray *scene = [Access getProductSceneWithId:[temp objectForKey:@"id"]];
        
        [temp setValue:[album arrayByAddingObjectsFromArray:scene] forKey:@"album"];
        
        //取样板间
        NSMutableArray *room = [Access getProductInRoomWithId:[temp objectForKey:@"id"]];
        
        [temp setValue:room forKey:@"room"];
    }
}
...
-(void)updataProductAttribute
{
    ...
    if (temp)
    {
        [self updataProductAttributeWithIndex:currentIndex];
        ...
        NSString *html = @"<body style='margin:0px;color:#ffffff;font-size:13px;'><strong>";
        
        if (![self isNull:[temp objectForKey:@"function"]]) 
        {
            html = [html stringByAppendingFormat:@"<font color='#999999'>基本配置:</font><br/>%@<br/>",[temp objectForKey:@"function"]];
        }
        
        if (![self isNull:[temp objectForKey:@"packSize"]] || ![self isNull:[temp objectForKey:@"packSize2"]]) 
        {
            html = [html stringByAppendingFormat:@"<font color='#999999'>包装:</font><br/>"];
            
            if (![self isNull:[temp objectForKey:@"packSize"]])
            {
                html = [html stringByAppendingFormat:@"<font color='#999999'>单位尺寸:</font><br/>%@<br/>",[temp objectForKey:@"packSize"]];
            }
            
            if (![self isNull:[temp objectForKey:@"packSize2"]]) 
            {
                html = [html stringByAppendingFormat:@"<font color='#999999'>三位尺寸:</font><br/>%@<br/>",[temp objectForKey:@"packSize2"]];
            }
        }
        
        if (![self isNull:[temp objectForKey:@"packBulk"]] || ![self isNull:[temp objectForKey:@"packBulk2"]]) 
        {
            html = [html stringByAppendingFormat:@"<font color='#999999'>体积:</font><br/>"];
            
            if (![self isNull:[temp objectForKey:@"packSize"]])
            {
                html = [html stringByAppendingFormat:@"<font color='#999999'>单位体积:</font><br/>%@<br/>",[temp objectForKey:@"packBulk"]];
            }
            
            if (![self isNull:[temp objectForKey:@"packSize2"]]) 
            {
                html = [html stringByAppendingFormat:@"<font color='#999999'>三位体积:</font><br/>%@<br/>",[temp objectForKey:@"packBulk2"]];
            }
        }
        
        if (![self isNull:[temp objectForKey:@"packRate"]]) 
        {
            html = [html stringByAppendingFormat:@"<font color='#999999'>包装率:</font><br/>%@<br/>",[temp objectForKey:@"packRate"]];
        }
        
        if (![self isNull:[temp objectForKey:@"idea"]]) 
        {
            html = [html stringByAppendingFormat:@"<font color='#999999'>设计理念:</font><br/>%@<br/>",[temp objectForKey:@"idea"]];
        }
        
        if (![self isNull:[temp objectForKey:@"materialDesc"]]) 
        {
            html = [html stringByAppendingFormat:@"<font color='#999999'>主要原材料:</font><br/>%@",[temp objectForKey:@"materialDesc"]];
        }
        
        html = [html stringByAppendingFormat:@"</font></strong></body>"];
        ...
        /*NSString *html = [NSString stringWithFormat:@"<body style='margin:0px;color:#ffffff;font-size:13px;'><strong>"\
                          ...
                          "<font color='#999999'>包装:</font><br/>"\
                          "<font color='#999999'>单位尺寸:</font><br/>%@<br/>"\
                          "<font color='#999999'>三位尺寸:</font><br/>%@<br/>"\
                          "<font color='#999999'>体积:</font><br/>"\
                          "<font color='#999999'>单位体积:</font><br/>%@<br/>"\
                          "<font color='#999999'>三位体积:</font><br/>%@<br/>"\
                          ...
                          [temp objectForKey:@"packSize2"],
                          [temp objectForKey:@"packBulk"],
                          [temp objectForKey:@"packBulk2"],
        ...
        //取总价
        float price = [[temp objectForKey:@"price"] floatValue];
        
        price += [[curCol objectForKey:@"price"] floatValue];
        
        price += [[curSpe objectForKey:@"price"] floatValue];
...
-(void)selectTouch:(UIButton*)sender
{
    if (sender.selected)
    {
        ...
        NSMutableDictionary *search = [NSMutableDictionary dictionaryWithObjectsAndKeys:[temp objectForKey:@"color"],@"color",
                                       [curCol objectForKey:@"id"],@"colorId",nil];
...
- (void)roomTouch:(id)sender
{
    ...
    if(room)
    {
        [window open:[MSRequest requestWithName:@"ProductInRoomController" search:room]];
        
        //NSArray *arr = [Access getRoom:[room intValue]];

        //window.location = [MSRequest requestWithName:@"RoomViewController" search:[arr objectAtIndex:0]];
    }
}
...
-(UIGalleryViewCell*)galleryView:(UIGalleryView *)galleryView cellForRowAtIndexPath:(NSIndex *)indexPath
{
    ...
    [self updataProductAttributeWithIndex:index];
    
    id current = [products objectAtIndex:index];
