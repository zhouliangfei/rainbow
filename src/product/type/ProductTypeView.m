...
{
    page = floorf((scrollView.contentOffset.x+512) / 1024.0);
}
...
-(void)galleryView:(UIGalleryView *)galleryView didSelectRowAtIndexPath:(NSIndex *)indexPath
{
    ...
    window.location.hash = [NSNumber numberWithInt:page];
...
{
    [flowCover setContentOffset:CGPointMake(page * 1024, 0)];
...
-(UIGalleryViewCell*)galleryView:(UIGalleryView *)galleryView cellForRowAtIndexPath:(NSIndex *)indexPath
{
    ...
    [cell.titleView setText:[NSString stringWithFormat:@"%@",[value objectForKey:@"model"]]];
