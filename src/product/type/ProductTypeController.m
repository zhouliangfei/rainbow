...
- (void)viewDidLoad
{
    ...
    if (window.location.search)
    {
        ...
        {
            page = [window.location.hash intValue];
        }
...
-(void)getFilterProduct
{
    ...
    if (filterType == 0)
    {
        ...
        for (NSDictionary *dic in function)
        {
            if ([dic objectForKey:@"select"] && [[dic objectForKey:@"select"] boolValue])
            {
                ...
                if ([name isEqualToString:@"大班椅"])
                {
                    [funArr addObject:@"big=1"];
                }
                
                if ([name isEqualToString:@"中班椅"])
                {
                    [funArr addObject:@"middle=1"];
                }
                
                if ([name isEqualToString:@"职员椅"])
                {
                    [funArr addObject:@"worker=1"];
                }
                
                if ([name isEqualToString:@"会客椅"])
                {
                    [funArr addObject:@"customer=1"];
                }
                
                if ([name isEqualToString:@"儿童椅"])
                {
                    [funArr addObject:@"child=1"];
                }
                
                if ([name isEqualToString:@"吧椅"])
                {
                    [funArr addObject:@"bar=1"];
                }
                
                if ([name isEqualToString:@"休闲椅"])
                {
                    [funArr addObject:@"arder=1"];
                }
                
                if ([name isEqualToString:@"功能椅"])
                {
                    [funArr addObject:@"function=1"];
                }
                
                if ([name isEqualToString:@"不锈钢椅"])
                {
                    [funArr addObject:@"stainless=1"];
                }
                
                if ([name isEqualToString:@"网椅"])
                {
                    [funArr addObject:@"net=1"];
                }
                
                if ([name isEqualToString:@"公共椅"])
                {
                    [funArr addObject:@"publik=1"];
                }
            }
        }
        ...
        if ([funArr count] > 0)
        {
            ...
            for (id val in funArr) 
            {
                if (arg == nil)
                {
                    arg = [NSString stringWithFormat:@"%@",val];
                }
                else
                {
                    arg = [arg stringByAppendingFormat:@" OR %@",val];
                }
            }
            
            filterSQL = [filterSQL stringByAppendingFormat:@"AND (%@)",arg];
        }
    }
    ...
    if (filterType == 1)
    {
        for (NSDictionary *dic in filters)
        {
            if ([dic objectForKey:@"select"] && [[dic objectForKey:@"select"] boolValue])
            {
                filterSQL = [filterSQL stringByAppendingFormat:@"AND material_id=%@ ",[dic objectForKey:@"id"]];
                
                for (NSDictionary *val in [dic objectForKey:@"value"])
                {
                    if ([val objectForKey:@"select"] && [[val objectForKey:@"select"] boolValue])
                    {
                        filterSQL = [filterSQL stringByAppendingFormat:@"AND size_id=%@ ",[val objectForKey:@"id"]];
    ...
    if (filterType == 2)
    {
        for (NSDictionary *dic in filters)
        {
            if ([dic objectForKey:@"select"] && [[dic objectForKey:@"select"] boolValue])
            {
                filterSQL = [filterSQL stringByAppendingFormat:@"AND size_id=%@ ",[dic objectForKey:@"id"]];
