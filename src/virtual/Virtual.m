...
- (void)viewDidLoad
{
    ...
    //
    [self.view addSubview:plView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 1024, 768)]];
...
- (void)initThumbView
{
    ...
    content = [[UIScrollView alloc] initWithFrame:CGRectMake(30, 83, 684, 143)];
    ...
    //这儿
    [self.view addSubview:map=[global imageWithSource:nil frame:CGRectMake(742, 573, 280, 196)]];
...
- (void)updata:(NSString*)value
{
    ...
    [content scrollRectToVisible:CGRectMake(0, 0, 684, 143) animated:YES];   
    
    //
    Thumbnail *item =  (Thumbnail *)[self.view viewWithTag:1];
...
- (void)thumbClick:(Thumbnail *)sender
{
    for(int i =0;i<[points count];i++){
        ...
        Thumbnail *thumb =  (Thumbnail *)[self.view viewWithTag:i+1];
    ...
    [pano addCubeTexture:[self getTexture:[NSString stringWithFormat:@"%@/%@/%@_",path,[item objectForKey:@"path"],[item objectForKey:@"path"]]]];
...
- (id)thumbWithFile:(NSString *)file lable:(NSString *)lable frame:(CGRect)frame event:(SEL)event
{
    ...
    [temp.titleLabel setFrame:CGRectMake(0, 0, frame.size.width, 30)];
